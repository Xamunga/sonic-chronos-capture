...
const SpectrumAnalyzer = () => {
  ...
  return <Card className="bg-gradient-to-br from-studio-charcoal to-studio-slate border-studio-electric/30">
      <div className="p-6">
        ...
        <div className="h-32 flex items-end justify-between space-x-1 bg-studio-dark rounded-lg p-2">
          {spectrum.map((level, index) => <div key={index} className="spectrum-bar w-2 rounded-t transition-all duration-75 ease-out" style={{
          height: `${level}%`
        }} />)}
        </div>
        ...
        <div className="mt-2 text-xs text-center text-muted-foreground">
          {audioService.isCurrentlyRecording() ? audioService.hasAudioSignal() ? <span className="text-studio-neon">Processamento Otimizado Windows 10/11</span> : <span className="text-studio-electric bg-studio-dark p-2 rounded border border-studio-electric/20">🎤 GRAVANDO - Aguardando sinal</span> : <span className="text-studio-warning bg-studio-dark p-2 rounded border border-studio-electric/20 ml-[50%]">SEM SINAL</span>}
        </div>
      </div>
    </Card>;
};
